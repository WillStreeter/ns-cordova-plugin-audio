 ...
 * It is called by the AudioHandlerNS Cordova class.
 ...
 */
public class AudioPlayerNS implements OnCompletionListener, OnPreparedListener, OnErrorListener {

    // AudioPlayerNS modes
    public enum MODE { NONE, PLAY, RECORD };

    // AudioPlayerNS states
    ...
    private static final String LOG_TAG = "AudioPlayerNS";

    // AudioPlayerNS message ids
...
//    private static int MEDIA_ERR_NONE_SUPPORTED = 4;
    ...
    private AudioHandlerNS handler;           // The AudioHandlerNS object
     ...
     */
    public AudioPlayerNS(AudioHandlerNS handler, String id, String file) {
    ...
    public void startRecording(String file) {
        switch (this.mode) {
        case PLAY:
            Log.d(LOG_TAG, "AudioPlayerNS Error: Can't record in play mode.");
        ...
        case NONE:
            ...
            break;
        case RECORD:
            Log.d(LOG_TAG, "AudioPlayerNS Error: Already recording.");
            sendErrorStatus(MEDIA_ERR_ABORTED);
        }
    }
    ...
    public void pausePlaying() {
        ...
        if (this.state == STATE.MEDIA_RUNNING && this.player != null) {
            ...
            Log.d(LOG_TAG, "AudioPlayerNS Error: pausePlaying() called during invalid state: " + this.state.ordinal());
    ...
    public void stopPlaying() {
        if ((this.state == STATE.MEDIA_RUNNING) || (this.state == STATE.MEDIA_PAUSED)) {
            ...
            Log.d(LOG_TAG, "AudioPlayerNS Error: stopPlaying() called during invalid state: " + this.state.ordinal());
            sendErrorStatus(MEDIA_ERR_NONE_ACTIVE);
        }
    }
    ...
    public boolean onError(MediaPlayer player, int arg1, int arg2) {
        Log.d(LOG_TAG, "AudioPlayerNS.onError(" + arg1 + ", " + arg2 + ")");
    ...
    private void setMode(MODE mode) {
        if (this.mode != mode) {
            ...
            //this.handler.webView.sendJavascript("cordova.require('ns-cordova-plugin-audio.NSmedia').onStatus('" + this.id + "', " + MEDIA_STATE + ", " + mode + ");");
        }
        this.mode = mode;
    }
    ...
    private boolean playMode() {
        ...
        case PLAY:
            break;
        case RECORD:
            Log.d(LOG_TAG, "AudioPlayerNS Error: Can't play in record mode.");
            sendErrorStatus(MEDIA_ERR_ABORTED);
        ...
        if (playMode()) {
            switch (this.state) {
                ...
                case MEDIA_LOADING:
                    ...
                    Log.d(LOG_TAG, "AudioPlayerNS Loading: startPlaying() called during media preparation: " + STATE.MEDIA_STARTING.ordinal());
                    this.prepareOnly = false;
                    return false;
                case MEDIA_STARTING:
                case MEDIA_RUNNING:
                ...
                default:
                    Log.d(LOG_TAG, "AudioPlayerNS Error: startPlaying() called during invalid state: " + this.state);
                    sendErrorStatus(MEDIA_ERR_ABORTED);
            }
        }
        return false;
